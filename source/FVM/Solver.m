...
% N.B. This improves efficiency at the expense of data redundancy.
...
for i = 1:timeSteps
    ...
    if (theta ~= 1)
        F_forwardEuler = dt * (1 - theta) * b(previousSolution, nodeCount, rows, columns, ...
            Vx, Vy, Dxx, Dyy, ...
...
n_phi(n_indices) = n_phi(n_indices) + nodeWidths(columnForIndex(n_indices)) ...
    .* (Vy(phi(n_indices)) .* (phi(n_indices - 1) + phi(n_indices)) ./ 2 ...
    - Dyy(phi(n_indices)) .* (phi(n_indices - 1) - phi(n_indices)) ...
    ./ yNodeDeltas(rowForIndex(n_indices) - 1));

n_phi(n_boundary_indices) = n_phi(n_boundary_indices) + nodeWidths(columnForIndex(n_boundary_indices)) ...
...
e_phi(e_indices) = e_phi(e_indices) + nodeHeights(rowForIndex(e_indices)) ...
    .* (Vx(phi(e_indices)) .* (phi(e_indices + rows) + phi(e_indices)) ./ 2 ...
    - Dxx(phi(e_indices)) .* (phi(e_indices + rows) - phi(e_indices)) ...
    ./ xNodeDeltas(columnForIndex(e_indices))); 

e_phi(e_boundary_indices) = e_phi(e_boundary_indices) + nodeHeights(rowForIndex(e_boundary_indices)) ...
...
s_phi(s_indices) = s_phi(s_indices) - nodeWidths(columnForIndex(s_indices)) ...
    .* (Vy(phi(s_indices)) .* (phi(s_indices + 1) + phi(s_indices)) ./ 2 ...
    - Dyy(phi(s_indices)) .* (phi(s_indices) - phi(s_indices + 1)) ...
    ./ yNodeDeltas(rowForIndex(s_indices)));

s_phi(s_boundary_indices) = s_phi(s_boundary_indices) - nodeWidths(columnForIndex(s_boundary_indices)) ...
...
w_phi(w_indices) = w_phi(w_indices) - nodeHeights(rowForIndex(w_indices)) ...
    .* (Vx(phi(w_indices)) .* (phi(w_indices - rows) + phi(w_indices)) ./ 2 ...
    - Dxx(phi(w_indices)) .* (phi(w_indices) - phi(w_indices - rows)) ...
    ./ xNodeDeltas(columnForIndex(w_indices) - 1));

w_phi(w_boundary_indices) = w_phi(w_boundary_indices) - nodeHeights(rowForIndex(w_boundary_indices)) ...
