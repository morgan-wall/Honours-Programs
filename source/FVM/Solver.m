...
for i = 1:timeSteps
    ...
    if (theta ~= 1)
        ...
        F_forwardEuler = F_forwardEuler ...
            - dt .* (1 - theta) .* source(previousSolution, nodesX(columnForIndex(indices)), nodesY(rowForIndex(indices)), times(i+1));
    end
    ...
    if (i == FIRST_TIME_STEP_INDEX)
        ...
        F_current_backwardEuler = F_current_backwardEuler ...
            - dt .* theta .* source(previousSolution, nodesX(columnForIndex(indices)), nodesY(rowForIndex(indices)), times(i+1));
    ...
    if (i == FIRST_TIME_STEP_INDEX)        
        [jacobian, jacobianNonlinearFnCalls] = ...
            GenerateJacobian(indices, nodeCount, previousSolution, dt, theta, ...
    ...
    while (current_iteration <= newtonParameters.maxIterations ...
        ...
        if (current_iteration ~= 0 && mod(current_iteration, chordSteps) == 0)
            ...
            [jacobian, jacobianNonlinearFnCalls] = ...
                GenerateJacobian(indices, nodeCount, currentSolution, dt, ...
        ...
        F_current_backwardEuler = dt .* theta ...
            .* GenerateFluxVec(preGlobalisationSolution, indices, rows, Vx, Vy, ...
            Dxx, Dyy, nodeWidths, nodeHeights, rowForIndex, columnForIndex, ...
            xNodeDeltas, yNodeDeltas, northBC, eastBC, southBC, westBC, ...
            isUpwinding, isNBoundaryIndex, isEBoundaryIndex, ...
            isSBoundaryIndex, isWBoundaryIndex, nodesX, nodesY, times(i+1));
        F_current_backwardEuler = F_current_backwardEuler ...
            - dt .* theta .* source(preGlobalisationSolution, nodesX(columnForIndex(indices)), nodesY(rowForIndex(indices)), times(i+1));
        F_current_backwardEuler = F_current_backwardEuler + preGlobalisationSolution;
        nonlinearFnCalls = nonlinearFnCalls + 1;
        ...
        if (isGlobalised)
            [Fx, delta_x, residualError, fCalls] = GlobaliseSolution(...
                currentSolution, residualError, delta_x, ...
                linesearchParam, minLambda, maxLambda, maxBacktracks, Fx, Fx_previous, ...
                dt, theta, source, F_forwardEuler, indices, rows, Vx, Vy, Dxx, Dyy, nodeWidths, ...
...
function [Fx, deltaX, etaK, nonlinearFnCalls] = GlobaliseSolution(...
    previousSolution, etaK, deltaX, t, minLambda, maxLambda, maxIterations, Fx, Fx_previous, ...
    dt, theta, source, F_forwardEuler, indices, rows, Vx, Vy, Dxx, Dyy, nodeWidths, nodeHeights, ...
    ...
    isSBoundaryIndex, isWBoundaryIndex, nodesX, nodesY, time)
...
while (norm(Fx) > (1 - t * (1 - etaK)) * norm(Fx_previous) && iteration < maxIterations)
    ...
    F_current_backwardEuler = dt .* theta ...
        .* GenerateFluxVec(testSolution, indices, rows, ...
        Vx, Vy, Dxx, Dyy, nodeWidths, nodeHeights, rowForIndex, ...
        columnForIndex, xNodeDeltas, yNodeDeltas, northBC, eastBC, southBC, ...
        westBC, isUpwinding, isNBoundaryIndex, isEBoundaryIndex, ...
        isSBoundaryIndex, isWBoundaryIndex, nodesX, nodesY, time);
    F_current_backwardEuler = F_current_backwardEuler ...
        - dt .* theta .* source(testSolution, nodesX(columnForIndex(indices)), nodesY(rowForIndex(indices)), time);
...
for i = 0:rows+initialOffset
    ...
    F_backwardEuler_stepped = F_backwardEuler_stepped ...
        - dt * theta * source(previousSolution, nodesX(columnForIndex(impactedNodes)), nodesY(rowForIndex(impactedNodes)), t);
...
if (~isempty(nIndices))
    ...
    flux(nIndices) = flux(nIndices) + nodeWidths(columnForIndex(nIndices)) ...
        .* (((Vy(nPhi, nodesX(columnForIndex(nIndices)), nodesY(rowForIndex(nIndices)), t) + Vy(northNeighbourPhi, nodesX(columnForIndex(nIndices - 1)), nodesY(rowForIndex(nIndices - 1)), t)) ./ 2) .* advectionAtFace ...
        - ((Dyy(nPhi, nodesX(columnForIndex(nIndices)), nodesY(rowForIndex(nIndices)), t) + Dyy(northNeighbourPhi, nodesX(columnForIndex(nIndices - 1)), nodesY(rowForIndex(nIndices - 1)), t)) ./ 2) .* (northNeighbourPhi - nPhi) ...
        ./ yNodeDeltas(rowForIndex(nIndices) - 1));
end
    
if (~isempty(nBoundaryIndices));    
    nBoundaryPhi = phi(nBoundaryIndices);
    diffusion = Dyy(nBoundaryPhi, nodesX(columnForIndex(nBoundaryIndices)), nodesY(rowForIndex(nBoundaryIndices)), t);
...
if (~isempty(eIndices))
    ...
    advectionVelocityX = Vx(phi(eIndices), nodesX(columnForIndex(eIndices)), nodesY(rowForIndex(eIndices)), t);
    if (isUpwinding)
    ...
    flux(eIndices) = flux(eIndices) + nodeHeights(rowForIndex(eIndices)) ...
        .* (((Vx(ePhi, nodesX(columnForIndex(eIndices)), nodesY(rowForIndex(eIndices)), t) + Vx(eastNeighbourPhi, nodesX(columnForIndex(eIndices + rows)), nodesY(rowForIndex(eIndices + rows)), t)) ./ 2) .* advectionAtFace ...
        - ((Dxx(ePhi, nodesX(columnForIndex(eIndices)), nodesY(rowForIndex(eIndices)), t) + Dxx(eastNeighbourPhi, nodesX(columnForIndex(eIndices + rows)), nodesY(rowForIndex(eIndices + rows)), t)) ./ 2) .* (eastNeighbourPhi - ePhi) ...
        ./ xNodeDeltas(columnForIndex(eIndices))); 
end

if (~isempty(eBoundaryIndices))    
    eBoundaryPhi = phi(eBoundaryIndices);
    diffusion = Dxx(eBoundaryPhi, nodesX(columnForIndex(eBoundaryIndices)), nodesY(rowForIndex(eBoundaryIndices)), t);
...
if (~isempty(sIndices))
    ...
    advectionVelocityY = Vy(phi(sIndices), nodesX(columnForIndex(sIndices)), nodesY(rowForIndex(sIndices)), t);
    if (isUpwinding)
        positiveAdvection = advectionVelocityY > 0;
    ...
    flux(sIndices) = flux(sIndices) - nodeWidths(columnForIndex(sIndices)) ...
        .* (((Vy(sPhi, nodesX(columnForIndex(sIndices)), nodesY(rowForIndex(sIndices)), t) + Vy(southNeighbourPhi, nodesX(columnForIndex(sIndices + 1)), nodesY(rowForIndex(sIndices + 1)), t)) ./ 2) .* advectionAtFace ...
        - ((Dyy(sPhi, nodesX(columnForIndex(sIndices)), nodesY(rowForIndex(sIndices)), t) + Dyy(southNeighbourPhi, nodesX(columnForIndex(sIndices + 1)), nodesY(rowForIndex(sIndices + 1)), t)) ./ 2) .* (sPhi - southNeighbourPhi) ...
        ./ yNodeDeltas(rowForIndex(sIndices)));
end
   
if (~isempty(sBoundaryIndices))
    sBoundaryPhi = phi(sBoundaryIndices);
    diffusion = Dyy(sBoundaryPhi, nodesX(columnForIndex(sBoundaryIndices)), nodesY(rowForIndex(sBoundaryIndices)), t);
...
if (~isempty(wIndices))
    ...
    advectionVelocityX = Vx(phi(wIndices), nodesX(columnForIndex(wIndices)), nodesY(rowForIndex(wIndices)), t);
    if (isUpwinding)
        positiveAdvection = advectionVelocityX > 0;
    ...
    flux(wIndices) = flux(wIndices) - nodeHeights(rowForIndex(wIndices)) ...
        .* (((Vx(wPhi, nodesX(columnForIndex(wIndices)), nodesY(rowForIndex(wIndices)), t) + Vx(westNeighbourPhi, nodesX(columnForIndex(wIndices - rows)), nodesY(rowForIndex(wIndices - rows)), t)) ./ 2) .* advectionAtFace ...
        - ((Dxx(wPhi, nodesX(columnForIndex(wIndices)), nodesY(rowForIndex(wIndices)), t) + Dxx(westNeighbourPhi, nodesX(columnForIndex(wIndices - rows)), nodesY(rowForIndex(wIndices - rows)), t)) ./ 2) .* (wPhi - westNeighbourPhi) ...
        ./ xNodeDeltas(columnForIndex(wIndices) - 1));
end

if (~isempty(wBoundaryIndices))    
    wBoundaryPhi = phi(wBoundaryIndices);
    diffusion = Dxx(wBoundaryPhi, nodesX(columnForIndex(wBoundaryIndices)), nodesY(rowForIndex(wBoundaryIndices)), t);
