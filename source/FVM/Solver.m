...
if (~isempty(nIndices))
    ...
    flux(nIndices) = flux(nIndices) + nodeWidths(columnForIndex(nIndices)) ...
        .* (Vy(nPhi) .* advectionAtFace - ((Dyy(nPhi) + Dyy(nPhi - 1)) ./ 2) ...
        .* (phi(nIndices - 1) - nPhi) ...
...
if (~isempty(eIndices))
    ...
    flux(eIndices) = flux(eIndices) + nodeHeights(rowForIndex(eIndices)) ...
        .* (Vx(ePhi) .* advectionAtFace - ((Dxx(ePhi) + Dxx(ePhi + rows)) ./ 2) ...
...
if (~isempty(sIndices))
    ...
    flux(sIndices) = flux(sIndices) - nodeWidths(columnForIndex(sIndices)) ...
        .* (Vy(sPhi) .* advectionAtFace - ((Dyy(sPhi) + Dyy(sPhi + 1)) ./ 2) ...
        .* (sPhi - phi(sIndices + 1)) ...
...
if (~isempty(wIndices))
    ...
    flux(wIndices) = flux(wIndices) - nodeHeights(rowForIndex(wIndices)) ...
        .* (Vx(wPhi) .* advectionAtFace - ((Dxx(wPhi) + Dxx(wPhi - rows)) ./ 2) ...
...
if (row == MIN_INDEX)
    ...
    flux = flux + cv_width * (cv_Vy * advectionAtFace ...
...
if (column == columns)
    ...
    flux = flux + cv_height * (cv_Vx * advectionAtFace ...
