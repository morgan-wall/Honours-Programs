...
for i = 1:timeSteps
    ...
    if (theta ~= 1)
        F_forwardEuler = dt .* (1 - theta) ...
            ...
            isSBoundaryIndex, isWBoundaryIndex, nodesX, nodesY, times(i+1), nodesXPos, nodesYPos, nodeWidthsAll, ...
    nodeHeightsAll);
        F_forwardEuler = F_forwardEuler ...
            - dt .* (1 - theta) .* source(previousSolution, nodesXPos, nodesYPos, times(i+1));
    end
    ...
    nodeHeightsAll);
        F_current_backwardEuler = F_current_backwardEuler ...
            - dt .* theta .* source(previousSolution, nodesXPos, nodesYPos, times(i+1));
    ...
    nodeHeightsAll);
        F_current_backwardEuler = F_current_backwardEuler ...
            - dt .* theta .* source(preGlobalisationSolution, nodesXPos, nodesYPos, times(i+1));
...
while (norm(Fx) > (1 - t * (1 - etaK)) * norm(Fx_previous) && iteration < maxIterations)
    ...
    F_current_backwardEuler = dt .* theta ...
        ...
        isSBoundaryIndex, isWBoundaryIndex, nodesX, nodesY, time, nodesXPos, nodesYPos, nodeWidthsAll, ...
    nodeHeightsAll);
    F_current_backwardEuler = F_current_backwardEuler ...
...
for i = 0:rows+initialOffset
    ...
    F_backwardEuler_stepped = F_backwardEuler_stepped ...
        - dt * theta * source(previousSolution, nodesXPos(impactedNodes), nodesYPos(impactedNodes), t);
...
function flux = GenerateFluxVec(phi, indices, rows, Vx, Vy, Dxx, Dyy, ...
    ...
    isWBoundaryIndex, nodesX, nodesY, t, nodesXPos, nodesYPos, nodeWidthsAll, ...
    nodeHeightsAll)
...
Vx_eval = Vx(phi, nodesXPos, nodesYPos, t);
Vy_eval = Vy(phi, nodesXPos, nodesYPos, t);

Dxx_eval = Dxx(phi, nodesXPos, nodesYPos, t);
Dyy_eval = Dyy(phi, nodesXPos, nodesYPos, t);
...
if (~isempty(nIndices))
    ...
    flux(nIndices) = flux(nIndices) + nodeWidthsAll(nIndices) ...
        .* (((Vy_eval(nIndices) + Vy_eval(nIndices - 1)) ./ 2) .* advectionAtFace ...
        - ((Dyy_eval(nIndices) + Dyy_eval(nIndices - 1)) ./ 2) .* (northNeighbourPhi - nPhi) ...
...
if (~isempty(nBoundaryIndices));    
    ...
    flux(nBoundaryIndices) = flux(nBoundaryIndices) ...
        ...
        .* ( (Vy_eval(nBoundaryIndices) + diffusion .* northBC.A(nodesXPos(nBoundaryIndices), t) ./ northBC.B(nodesXPos(nBoundaryIndices), t)) ...
        .* nBoundaryPhi - diffusion .* northBC.C(nodesXPos(nBoundaryIndices), t) ./ northBC.B(nodesXPos(nBoundaryIndices), t) );
end
...
if (~isempty(eIndices))
    ...
    flux(eIndices) = flux(eIndices) + nodeHeightsAll(eIndices) ...
        .* (((Vx_eval(eIndices) + Vx_eval(eIndices + rows)) ./ 2) .* advectionAtFace ...
        - ((Dxx_eval(eIndices) + Dxx_eval(eIndices + rows)) ./ 2) .* (eastNeighbourPhi - ePhi) ...
...
if (~isempty(eBoundaryIndices))    
    ...
    flux(eBoundaryIndices) = flux(eBoundaryIndices) ...
        ...
        .* ( (Vx_eval(eBoundaryIndices) + diffusion .* eastBC.A(nodesYPos(eBoundaryIndices), t) ./ eastBC.B(nodesYPos(eBoundaryIndices), t)) ...
        .* eBoundaryPhi - diffusion .* eastBC.C(nodesYPos(eBoundaryIndices), t) ./ eastBC.B(nodesYPos(eBoundaryIndices), t) );
end
...
if (~isempty(sIndices))
    ...
    flux(sIndices) = flux(sIndices) - nodeWidthsAll(sIndices) ...
        .* (((Vy_eval(sIndices) + Vy_eval(sIndices + 1)) ./ 2) .* advectionAtFace ...
        - ((Dyy_eval(sIndices) + Dyy_eval(sIndices + 1)) ./ 2) .* (sPhi - southNeighbourPhi) ...
...
if (~isempty(sBoundaryIndices))
    ...
    flux(sBoundaryIndices) = flux(sBoundaryIndices) ...
        ...
        .* ( (Vy_eval(sBoundaryIndices) - diffusion .* southBC.A(nodesXPos(sBoundaryIndices), t) ./ southBC.B(nodesXPos(sBoundaryIndices), t)) ...
        .* sBoundaryPhi + diffusion .* southBC.C(nodesXPos(sBoundaryIndices), t) ./ southBC.B(nodesXPos(sBoundaryIndices), t) );
end
...
if (~isempty(wIndices))
    ...
    flux(wIndices) = flux(wIndices) - nodeHeightsAll(wIndices) ...
        .* (((Vx_eval(wIndices) + Vx_eval(wIndices - rows)) ./ 2) .* advectionAtFace ...
        - ((Dxx_eval(wIndices) + Dxx_eval(wIndices - rows)) ./ 2) .* (wPhi - westNeighbourPhi) ...
...
if (~isempty(wBoundaryIndices))    
    ...
    flux(wBoundaryIndices) = flux(wBoundaryIndices) ...
        ...
        .* ( (Vx_eval(wBoundaryIndices) - diffusion .* westBC.A(nodesYPos(wBoundaryIndices), t) ./ westBC.B(nodesYPos(wBoundaryIndices), t)) ...
