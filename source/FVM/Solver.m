...
nBoundaryIndices = indices(mod(indices - 1, rows) == 0);
...
wBoundaryIndices = indices(1:rows);
wIndices = indices(rows+1:end);
...
for i = 1:timeSteps
    ...
    isUpwinding, nBoundaryIndices, nIndices, eBoundaryIndices, eIndices, ...
...
if (~isempty(nIndices))
    ...
    flux(nIndices) = flux(nIndices) + nodeWidths(columnForIndex(nIndices)) ...
        .* (Vy(nPhi) .* advectionAtFace ...
        - Dyy(nPhi) .* (phi(nIndices - 1) - nPhi) ...
...
if (~isempty(eIndices))
    ...
    flux(eIndices) = flux(eIndices) + nodeHeights(rowForIndex(eIndices)) ...
        .* (Vx(ePhi) .* advectionAtFace ...
        - Dxx(ePhi) .* (phi(eIndices + rows) - ePhi) ...
...
if (~isempty(sIndices))
    ...
    flux(sIndices) = flux(sIndices) - nodeWidths(columnForIndex(sIndices)) ...
        .* (Vy(sPhi) .* advectionAtFace ...
        - Dyy(sPhi) .* (sPhi - phi(sIndices + 1)) ...
...
if (~isempty(wIndices))
    ...
    flux(wIndices) = flux(wIndices) - nodeHeights(rowForIndex(wIndices)) ...
        .* (Vx(wPhi) .* advectionAtFace ...
        - Dxx(wPhi) .* (wPhi - phi(wIndices - rows)) ...
