...
Dxx = @(phi, x, y, t) ones(length(phi), 1) * 0.1; 
Dxx = @(phi, x, y, t) phi .* 0;

Dyy = @(phi, x, y, t) ones(length(phi), 1) * 0.1; 
Dyy = @(phi, x, y, t) phi .* 0;

Vx = @(phi, x, y, t) phi .* 0; 
Vx = @(phi, x, y, t) ones(length(phi), 1) * 0.1;

Vy = @(phi, x, y, t) phi .* 0; 
Vy = @(phi, x, y, t) ones(length(phi), 1) * 0.1;
...
westA = @(y, t) y .* 0 + dirichletHackCoef;
westB = @(y, t) y .* 0 + 1;
westC = @(y, t) y .* 0 + dirichletHackCoef;
westBC = struct('A', westA, 'B', westB, 'C', westC);
...
northA = @(x, t) x .* 0 + dirichletHackCoef;
northB = @(x, t) x .* 0 + 1;
northC = @(x, t) x .* 0 + dirichletHackCoef;
northBC = struct('A', northA, 'B', northB, 'C', northC);
...
eastA = @(y, t) y .* 0 + dirichletHackCoef;
eastB = @(y, t) y .* 0 + 1;
eastC = @(y, t) y .* 0 + dirichletHackCoef;
eastBC = struct('A', eastA, 'B', eastB, 'C', eastC);
...
southA = @(x, t) x .* 0 + dirichletHackCoef;
southB = @(x, t) x .* 0 + 1;
southC = @(x, t) x .* 0 + dirichletHackCoef;
southBC = struct('A', southA, 'B', southB, 'C', southC);
